...
***Fig. 1:** The figure shows the use of the trigger and echo in an ultrasonic sensor. A pulse is sent out from the PWM pin, which is converted from a trigger into an acoustic burst that is sent out and returned and can vary depending on the distance of the nearest object. The gap between the outgoing and returning acoustic bursts is the PWM output from the echo pin, which can be converted into a distance. It is important to note that there is a needed delay between the pulses that are sent out for the sensor to work appropriately.*
